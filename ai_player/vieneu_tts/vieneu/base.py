...
from ._compat import import_vieneu_utils
...
class BaseVieneuTTS(ABC):
    ...
    def __init__(self, codec_repo: str | None = None, codec_device: str = "cpu"):
    ...
    def _load_codec(self, codec_repo: str, codec_device: str) -> None:
        ...
        try:
            ...
            if codec_path.exists() and codec_path.is_dir():
                model_path = codec_path / "pytorch_model.bin"
                hubert_path = codec_path / "distilhubert"
                if not model_path.exists():
                    raise FileNotFoundError(f"Local codec missing pytorch_model.bin: {codec_path}")
                if not (hubert_path / "config.json").exists():
                    raise FileNotFoundError(f"Local codec missing distilhubert files: {hubert_path}")
                self.codec = _load_local_distill_neucodec(DistillNeuCodec, hubert_path)
                ...
                if os.getenv("HF_HUB_OFFLINE") == "1":
                    self.codec = DistillNeuCodec._from_pretrained(
                        model_id=codec_repo,
    ...
    def _load_voices(
        self, backbone_repo: str | None, hf_token: str | None = None, clear_existing: bool = False
    ) -> None:
    ...
    def _load_voices_from_repo(self, repo_id: str, hf_token: str | None = None) -> None:
    ...
    def get_preset_voice(self, voice_name: str | None = None) -> dict[str, Any]:
        """
    ...
    def get_ref_phonemes(self, ref_text: str) -> str:
        ...
        if ref_text not in self._ref_phoneme_cache:
            phonemize_with_dict = import_vieneu_utils("phonemize_text").phonemize_with_dict

            self._ref_phoneme_cache[ref_text] = phonemize_with_dict(ref_text)
        return self._ref_phoneme_cache[ref_text]

    def save(self, audio: np.ndarray, output_path: str | Path) -> None:
    ...
    def encode_reference(self, ref_audio_path: str | Path) -> np.ndarray | torch.Tensor:
        ...
        try:
            ...
            raise ImportError(
                "Torch is required for encode_reference in the current backend. "
                "Please install torch or use a backend that supports standalone encoding."
            ) from exc
    ...
    def _decode(self, codes_str: str) -> np.ndarray:
        ...
        # Torch decode
            ...
            try:
                ...
                raise ImportError(
                    "Torch is required for the current codec backend. Please install torch or use an ONNX-based codec."
                ) from exc
    ...
    def _resolve_ref_voice(
        ...
        ref_codes: np.ndarray | torch.Tensor | None = None,
    ...
    def _format_prompt(
        ...
        ref_text: str,
        input_text: str,
        ref_phonemes: str | None = None,
        input_phonemes: str | None = None,
    ) -> str:
        """
...
def _load_local_distill_neucodec(distill_neucodec_cls, hubert_path: Path):
    from transformers import AutoFeatureExtractor, HubertModel

    original_hubert = HubertModel.from_pretrained
    original_extractor = AutoFeatureExtractor.from_pretrained

    def hubert_from_pretrained(model_id, *args, **kwargs):
        if model_id == "ntu-spml/distilhubert":
            return original_hubert(str(hubert_path), *args, local_files_only=True, **kwargs)
        return original_hubert(model_id, *args, **kwargs)

    def extractor_from_pretrained(model_id, *args, **kwargs):
        if model_id == "ntu-spml/distilhubert":
            return original_extractor(str(hubert_path), *args, local_files_only=True, **kwargs)
        return original_extractor(model_id, *args, **kwargs)

    HubertModel.from_pretrained = hubert_from_pretrained
    AutoFeatureExtractor.from_pretrained = extractor_from_pretrained
    try:
        return distill_neucodec_cls(24_000, 480)
    finally:
        HubertModel.from_pretrained = original_hubert
        AutoFeatureExtractor.from_pretrained = original_extractor
