...
class RemoteVieNeuTTS(BaseVieneuTTS):
    ...
    async def infer_async(
        ...
        try:
            ...
            raise ImportError("Async requires 'aiohttp'.") from exc
    ...
    async def _infer_chunk_async(
        ...
        session,
        chunk: str,
        ref_codes: list[int] | torch.Tensor | np.ndarray,
        ref_text: str,
        temperature: float,
        top_k: int,
        ref_phonemes: str | None = None,
        chunk_phonemes: str | None = None,
    ) -> np.ndarray:
    ...
    async def infer_batch_async(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async def bounded_infer(text, ph):
                async with sem:
                    ...
                    tasks = [
                        ...
                        for c, c_ph in zip(chunks, chunk_phonemes, strict=True)
                    ]
                    wavs = await asyncio.gather(*tasks)
                    final_wav = join_audio_chunks(wavs, self.sample_rate, silence_p, crossfade_p)
                    if apply_watermark:
            ...
            tasks = [bounded_infer(text, ph) for text, ph in zip(texts, all_phonemes, strict=True)]
