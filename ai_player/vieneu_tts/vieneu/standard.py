...
from .utils import _linear_overlap_add, normalize_device

logger = logging.getLogger("Vieneu.Standard")

core_utils = import_vieneu_utils("core_utils")
phonemize_text = import_vieneu_utils("phonemize_text")
join_audio_chunks = core_utils.join_audio_chunks
split_text_into_chunks = core_utils.split_text_into_chunks
phonemize_batch = phonemize_text.phonemize_batch
phonemize_with_dict = phonemize_text.phonemize_with_dict
...
class VieNeuTTS(BaseVieneuTTS):
    ...
    def _load_backbone(self, backbone_repo: str, backbone_device: str, hf_token: str | None = None) -> None:
        ...
        if backbone_repo.lower().endswith("gguf") or "gguf" in backbone_repo.lower():
            ...
            if backbone_path.exists() and backbone_path.is_file():
                self.backbone = Llama(model_path=str(backbone_path), **llama_kwargs)
            else:
    ...
    def load_lora_adapter(self, lora_repo_id: str, hf_token: str | None = None) -> bool:
        if self._is_quantized_model:
        ...
        try:
            ...
            raise ImportError("PEFT library required for LoRA. Install with: pip install peft") from exc
    ...
    def _infer_ggml(
        self, ref_codes: Any, ref_phonemes: str, chunk_phonemes: str, temperature: float = 1.0, top_k: int = 50
    ) -> str:
        ref_codes_list = self.to_list(ref_codes)
