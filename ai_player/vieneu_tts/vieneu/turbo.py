...
core_utils = import_vieneu_utils("core_utils")
phonemize_text_module = import_vieneu_utils("phonemize_text")
get_silence_duration_v2 = core_utils.get_silence_duration_v2
split_into_chunks_v2 = core_utils.split_into_chunks_v2
phonemize_batch = phonemize_text_module.phonemize_batch
phonemize_text = phonemize_text_module.phonemize_text
...
class BaseTurboVieNeuTTS(BaseVieneuTTS):
    ...
    def _load_decoder(self, decoder_repo, decoder_filename, device, hf_token=None):
        ...
        if os.path.exists(decoder_repo) and not os.path.isdir(decoder_repo):
            ...
            decoder_path = hf_hub_download(repo_id=decoder_repo, filename=decoder_filename, token=hf_token)
    ...
    def _load_encoder(self, encoder_repo, encoder_filename, device, hf_token=None):
        ...
        if os.path.exists(encoder_repo) and not os.path.isdir(encoder_repo):
            ...
            try:
                encoder_path = hf_hub_download(repo_id=encoder_repo, filename=encoder_filename, token=hf_token)
            except Exception:
    ...
    def _format_turbo_prompt(self, phonemes: str) -> str:
        return f"<|speaker_16|><|TEXT_PROMPT_START|>{phonemes}<|TEXT_PROMPT_END|><|SPEECH_GENERATION_START|>"
    ...
    def _decode(self, codes_str: str, voice_embedding: np.ndarray) -> np.ndarray:
        ...
        inputs = {"content_ids": tokens, "voice_embedding": voice_embedding}
...
class TurboGPUVieNeuTTS(BaseTurboVieNeuTTS):
    ...
    def _load_backbone(self, repo, device, hf_token=None, **kwargs):
        if self.backend == "lmdeploy":
            if self.device != "cuda":
                ...
                try:
                    ...
                    logger.info("✅ Turbo GPU (LMDeploy) ready")
                    return
                except ImportError:
        ...
        if self.backend == "standard":
            ...
            self.backbone.eval()
            logger.info("✅ Turbo GPU (Standard) ready")
    ...
    def infer(
        ...
        **kwargs,
        ...
        final_wav = (
            np.concatenate(all_wavs)
            if len(all_wavs) > 1
            else (all_wavs[0] if all_wavs else np.array([], dtype=np.float32))
        )
        if apply_watermark:
        ...
        for i in range(0, len(texts), max_batch_size):
            ...
            if self.backend == "lmdeploy":
                ...
                batch_wavs = [
                    ...
                    for t in texts[i : i + max_batch_size]
                ]
...
class TurboVieNeuTTS(BaseTurboVieNeuTTS):
    ...
    def _load_backbone(self, backbone_repo, backbone_filename, device, hf_token=None, **kwargs):
        try:
            ...
            raise ImportError("llama-cpp-python is required for Turbo mode.") from exc
        ...
        **kwargs,
        ...
        for i, chunk in enumerate(pbar):
            ...
            result = self.backbone(
                self._format_turbo_prompt(chunk.text),
