...
from ai_player.pipeline.export_plan import ExportCue, ExportRange
from ai_player.services.tts import is_non_speech_tts_text
from ai_player.workers.export_utils import _format_hhmmss, _probe_duration_seconds, _tts_disabled

MakeSilence = Callable[[float, Path], object]
TrimLeadingSilence = Callable[[Path], Path]
ToWav = Callable[[Path, Path], object]
BuildTranscriptExportCue = Callable[[int, object, str, str, float, float, str], ExportCue]
SetRangeProgress = Callable[[int, int, int, int], object]
EmitProgress = Callable[..., object]


def transcript_export_artifact_paths(temp_dir: Path, index: int, tts_suffix: str) -> tuple[Path, Path]:
    return temp_dir / f"transcript-cue-{index:05d}.{tts_suffix}", temp_dir / f"transcript-cue-{index:05d}.wav"


def build_transcript_export_cue(
    *,
    index: int,
    original: str,
    translated: str,
    ...
    tts_path, wav_path = transcript_export_artifact_paths(temp_dir, index, tts_suffix)
    ...
    if _tts_disabled(config) or is_non_speech_tts_text(translated):
        ...
        return ExportCue(
            start_seconds=export_range.shift(entry_start),
            original=original,
...
def build_prepared_transcript_export_cues(
    *,
    items: list[object],
    translated_items: list[str],
    tts_suffix: str,
    max_workers: int,
    build_transcript_export_cue: BuildTranscriptExportCue,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for completed, future in enumerate(as_completed(futures)):
            cue = future.result()
            cues.append(cue)
            set_range_progress(18, 74, completed, total_entries)
